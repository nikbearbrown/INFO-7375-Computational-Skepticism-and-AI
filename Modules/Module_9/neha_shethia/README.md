# January Weekly Report Summary  

---

## Week 1: Language Models Fundamentals
- Introduced **Language Models (LMs)** as systems that predict the next word/token in a sequence.
- Explained next-token prediction using probability-based examples.
- Covered the **training lifecycle**:
  - **Pre-training:** models learn language patterns by reading massive text corpora (books, websites, papers).
  - **Post-training / Fine-tuning:** human guidance (e.g., RLHF) teaches models to follow instructions and behave safely.
- Discussed research insight:
  - **Chain-of-Thought prompting** improves reasoning when models are encouraged to think step-by-step.

---

## Week 2: Limitations of Traditional Language Models
- Identified key limitations of standalone LMs:
  ...
  - **Lack of attribution:** answers often lack sources or citations.
  - **Limited interactivity:** no native access to tools, APIs, or databases.
  - **Context constraints:** limited ability to process very long documents.
- Introduced mitigation strategies:
  - **Retrieval-Augmented Generation (RAG)** for grounding answers in external data.
  - **Tool-use frameworks** (e.g., Toolformer) to enable external computation.
  - Long-context models and document chunking.

---

## Week 3: Emergence of Agentic AI
- Defined **Agentic AI** as systems that extend LMs with:
  - planning and task decomposition
  - memory/state across steps
  - external tool usage
  - iterative execution toward goals
- Discussed key research and systems:
  - **ReAct:** interleaves reasoning and action to reduce hallucination.
  - **Toolformer:** trains models to decide when to use tools.
  - **Auto-GPT:** autonomous task execution demo with observed failure modes.

---

## Week 4: Agent Frameworks and Practical Exercises
- Introduced the **agent loop**: *Perceive → Reason → Act → Learn*.
- Covered classical AI agent types:
  - Simple Reflex, Model-Based, Goal-Based, Utility-Based, Learning Agents.
- Conducted hands-on classroom activities:
  - Next-word prediction exercises.
  - Hallucination detection and fact-checking.
  - Mini **RAG lab** using a small document corpus.
  - **ReAct role-play** (Reasoning, Action, Memory, Answer).
- Emphasized real-world implications:
  - importance of guardrails in tool execution
  - feedback loops and data flywheels for improvement
  - connection between technical design and ethical responsibility.
